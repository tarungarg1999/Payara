...
git push ${BITBUCKET_REMOTE} CUSTCOM-${JIRA_NUMBER}-blue-${VERSION}.RC${RC_VERSION} --force
  
# Create Version Increment Branch
git branch -D CUSTCOM-${JIRA_NUMBER}-Increment-Version-Numbers-Blue-4.${NEXT_MINOR_VERSION}
git checkout payara-blue
git checkout -b CUSTCOM-${JIRA_NUMBER}-Increment-Version-Numbers-Blue-4.${NEXT_MINOR_VERSION}
...
# Commit and push
git add *
git commit -m "CUSTCOM-${JIRA_NUMBER} Increment version numbers"
git push ${GITHUB_REMOTE} CUSTCOM-${JIRA_NUMBER}-Increment-Version-Numbers-Blue-4.${NEXT_MINOR_VERSION} --force
