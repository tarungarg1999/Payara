...
sed -i "s/version>${OLD_VERSION}</version>${VERSION}</g" Payara-EJB-HTTP-Client-${VERSION}.pom
...
sed -i "s/description>Full Distribution of the Payara Project</description>Module provifing support for EJB HTTP Client</g" Payara-EJB-HTTP-Client/ejb-http-client-${VERSION}.pom
